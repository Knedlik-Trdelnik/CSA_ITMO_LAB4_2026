...
Устройство управления — **Hardwired (`hw`)**. Процессор представляет собой конечный автомат, моделируемый с точностью до такта (`tick`). Машинный код представляет собой чистые бинарные данные (`binary`).
...
### Аппаратный вход в прерывание (Context Switch)
...
- **DataPath**: Изолированная модель передачи данных. Содержит АЛУ, регистр A (как Address Buffer), MUX-коммутаторы и строго фиксированные массивы `stack` и `return_stack`. Вырезан регистр B за ненадобностью (АЛУ работает напрямую с вершинами Data Stack).
- **ControlUnit**: Модель конечного автомата. Выполняет инструкции по микрошагам `self.step`. Обновляет глобальный счетчик тактов `self._tick`.
...
Вы можете передать имя файла через аргумент командной строки (без расширения `.txt`):
```bash
...
Для автоматического обновления эталонных файлов (при изменении формата логов):
```bash
# Для Linux/macOS
UPDATE_GOLDEN=1 pytest tests/test_integration.py -v

# Для Windows (PowerShell)
$env:UPDATE_GOLDEN="1"; pytest tests/test_integration.py -v; $env:UPDATE_GOLDEN="0"
```
...
Настроен в файле `.github/workflows/python-app.yml`.
Автоматически проверяет стиль кода линтером `ruff`, осуществляет строгую проверку типов (`mypy`) и прогоняет интеграционные тесты (`pytest`). Любая ошибка форматирования или падение теста приводит к блокировке коммита.