...
Для обеспечения работы указателей `DataPath` оснащен специализированным аппаратным буфером — **Регистром А (`Reg A`)**. Его единственная жизненная цель — хранить адреса для косвенных обращений к памяти (а после их инкрементации).Раньше был страшный аналог - постоянно загружать адрес -> инкремент -> сохранение. 
